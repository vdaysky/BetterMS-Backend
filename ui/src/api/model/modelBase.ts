import {StateUpdateMessage} from "@/api/utils";
...
export function isDependencyAffected(dependency: Dependency<any, any>, model: Loadable, update: StateUpdateMessage) {
    ...
    if (dependency.predicate && !dependency.predicate(update as any, update.changes || {}, model.__myProxy as any)) {
        return false;
    }
...
abstract class Loadable {
    ...
    // abstract safeWaitLoaded(): Promise<void>;
     ...
     * SHOULD NOT START RECURSIVE RESOLUTION PROCESS. Returns ref to model wrapped by my proxy. */
    static getOrCreate (
        item: any,
        fieldName: string,
        field: FieldInfo,
...
type Constructor<T> = Model & { new (): T };
...
export class Model extends Loadable {
    ...
    static createView<T extends typeof Model>(this: T, deps: ObjID[]): T {
        const view = new this(
            {
                obj_id: null,
                entity: this.name,
...
export class ModelManager {
    ...
    makeReactive(modelKey: string, model: Model | Struct): this {
        const r = reactive(model);
        this.models[modelKey] = r as any;
        return r as any;
    }

    get(model: ObjID) {
        return this.models[objectIdToString(model)];
    }
}
